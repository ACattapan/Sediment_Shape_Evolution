function [H ADK ADKs ADKc P] = AnDarksamtestNum_Modified(X,alpha)
...
%
clear k N c v nc hj hjn O zj L zjn Ln n Fij Fijn Hj Hjn 
switch nargin
    case{2}
...
[c,v] = hist(sort(X1),sort(X1));    %c counts the values equal to each value in X1; 
                                    %if the are multiple items ci=2 or 3 while the 
                                    %consecutive value equals 0 because it was counted before etc v containes their value
nc = find(c~=0);                    %finds nonzero elements in c
c = [v(nc) c(nc)'];                 %saves each value of X1 and its count in c(1) and c(2) respectively
hj = c(:,2);                        %number of values in combined samples equal to zj
hjn = c(1:end-1,2);                 %number of values in combined samples equal to zj excluding the last value
O = sort(X1);                       %sort elements of X1
zj = unique(O);                     %distinct values in the combined data set ordered
L = length(zj);                     %Length of the dataset excluding multiple values
...
for i=1:k
    r = find(X2 == i);              %indices of elements belonging to group i
    ni = length(r);                 %number of observations in sample i
    n = [n ni];                     %attach the new column ni to the right side of n
    if (any(ni==0))
        error('One or more samples have no observations.');
    end
    rx = X1(r);                     %rx containes the observations of the ith group
    [x1,x2] = ndgrid(rx,zj);        %creates a grid with the size of ith observations and sorted unique values in X1
    [x1n,x2n] = ndgrid(rx,zjn);
    fij = sum(x1==x2);              %creates a vector with 1 in the list of sorted unique values
    fijn = sum(x1n==x2n);
    ...
    Hjn = [cumsum(hjn)];            %number of values in the combined samples less
                                    ...
                                    %adjusted for ties.
%     if size(N*Fij)~=size(ni*Hj)
%         %          save('ADKtestDebug.mat','N','Fij','ni','Hj');
%         N
%         Fij
%         ni
%         Hj
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Optional k-sample Anderson-Darling Hypothesis Output
if P >= alpha;
    H=false(1);
else
    H=true(1);
