function Circularity_Fit_Model(X,Parameter,Distance_Units,Distance_Metric,Fitting_Param,FitTest,CLowerC0,PathNum,Circularity_Fit_Model_file,mu_max,C_max,C0,a,Ka)
% 
%--------------------------------------------------------------------------
arguments
    X (:,:) cell
...
RMSE_C_L=zeros(size(C0,2),size(a,2),size(Ka,2));
clear mu_L C_L
for i=1:size(C0,2)
    for j=1:size(a,2)
        for k=1:size(Ka,2)
            ...
            C_L=C_of_mu_func(mu_L,mu_max,C0(i),C_max,a(j));

            RMSE_C_L(i,j,k)=rmse(C_L,C_E);
            clear mu_L C_L
        end
    end
end
...
Opt_Param_C_L{1}=C0(I1_C_L);
Opt_Param_C_L{2}=a(I2_C_L);
Opt_Param_C_L{3}=Ka(I3_C_L);
Opt_Param_C_L{4}=RMSE_C_L(I1_C_L,I2_C_L,I3_C_L);
%--------------------------------------------------------------------------
...
mu_L_plot=mu_of_L_func(L_plot,mu_max,Ka(I3_C_L));

% C_L=C0(I1_C_L)+(C_max-C0(I1_C_L)).*(1-(1-mu_L_plot./mu_max).^a(I2_C_L)).^(1/a(I2_C_L));
C_L_plot=C_of_mu_func(mu_L_plot,mu_max,C0(I1_C_L),C_max,a(I2_C_L));
...
mu_L_scatter=mu_of_L_func(L,mu_max,Ka(I3_C_L));

C_L_scatter=C_of_mu_func(mu_L_scatter,mu_max,C0(I1_C_L),C_max,a(I2_C_L));
